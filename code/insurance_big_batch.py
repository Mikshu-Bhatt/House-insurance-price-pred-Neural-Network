...
train_loader = DataLoader(dataset=train_dataset, batch_size=len(test_dataset), shuffle=True)
...
l = int(len(train_dataset)/len(test_dataset))
l_test = int(len(test_dataset)/len(test_dataset))
min_t = np.Inf
min_val = np.Inf
t_loss = []
v_loss = []
for epoch in range(300):
    ...
    with torch.no_grad():
        ...
        v_loss.append(loss1.item()/l_test)
