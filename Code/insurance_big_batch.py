...
train_loader = DataLoader(dataset=train_dataset, batch_size=300000, shuffle=True)
test_dataset = TensorDataset(X_test, y_test)
test_loader = DataLoader(dataset=test_dataset, batch_size=100000, shuffle=True)
...
for epoch in range(150):
    ...
    print(f'train_loss for {epoch} is ---> {loss0/l_tr} and val loss is {loss1/l_tst}')
    if loss0/l < 0.00001:
...
plt.legend()
plt.show()
plt.savefig('big_batch.png')


torch.save(best_model.state_dict(), '/home/scai/mtech/aib232067/ML/best_model_big_batch.pth')
